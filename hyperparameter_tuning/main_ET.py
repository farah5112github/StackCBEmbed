...
c = Counter(y)
print(c)
#n_estimators=100, max_depth=None, max_features='sqrt', bootstrap=False, criterion='gini'
grid = {
...
et_cv = GridSearchCV(ExtraTreesClassifier(random_state=1), grid, cv=10, scoring=['accuracy', 'balanced_accuracy', 'precision', 'recall', 'f1', 'roc_auc', 'average_precision'], refit='f1')
et_cv.fit(X, y)
df = pd.DataFrame(et_cv.cv_results_)
df[['params', 'mean_test_accuracy', 'mean_test_balanced_accuracy', 'mean_test_precision', 'mean_test_recall', 'mean_test_f1', 'mean_test_roc_auc', 'mean_test_average_precision']].to_csv('./output_csvs/ET_grid_search_.csv', index=False)
