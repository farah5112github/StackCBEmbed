...
#hidden_layer_sizes=(100,), activation='relu', solver='adam', alpha=0.0001, learning_rate='constant',
grid = {
...
mlp_cv = GridSearchCV(MLPClassifier(random_state=1, max_iter=4000), grid, cv=10, scoring=['accuracy', 'balanced_accuracy', 'precision', 'recall', 'f1', 'roc_auc', 'average_precision'], refit='f1')
mlp_cv.fit(X, y)
df = pd.DataFrame(mlp_cv.cv_results_)
df[['params', 'mean_test_accuracy', 'mean_test_balanced_accuracy', 'mean_test_precision', 'mean_test_recall', 'mean_test_f1', 'mean_test_roc_auc', 'mean_test_average_precision']].to_csv('./output_csvs/MLP_grid_search_.csv', index=False)
