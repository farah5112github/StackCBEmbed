...
def load_the_pickle_files_base_layer(converted_all_features_with_output, folder_path):

    test_X = converted_all_features_with_output[:,1:] ;
    test_base_output_total = np.zeros((len(test_X),1), dtype=float);

    #print('test X : ',test_X.shape);
    pickle_folder_path = str( folder_path + 'necessary_pickle_files/' ) ;
...
def load_the_pickle_files_meta_layer(converted_all_features_with_output, folder_path):
    test_X = converted_all_features_with_output[:, 1:];
