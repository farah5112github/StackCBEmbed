...
with open('./output_csvs/metrics_TS49.csv', 'w') as f:
    ...
    for cmd in cmds:
        ...
        if cmd == '0':
            ...
            writer.writerow(['SStackCBEmbed', '{0:.4f}'.format(sensitivity), '{0:.4f}'.format(specificity), '{0:.4f}'.format(bal_acc), '{0:.4f}'.format(acc), '{0:.4f}'.format(f1_score_1), '{0:.4f}'.format(mcc), '{0:.4f}'.format(auc), '{0:.4f}'.format(auPR)])
        else:
