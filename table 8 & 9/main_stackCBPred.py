...
with open('./output_csvs/metrics_StackCBPred.csv', 'w') as f:
    ...
    for t in test_set:
        ...
        model = SVC(C=2.378414230005442, kernel='rbf', gamma=0.013139006488339289, probability=True)
        model.fit(X_scale_5, y_5)
        ...
        model = SVC(C=0.21022410381342863, gamma=0.0011613350732448448, probability=True)
        model.fit(X, y)
        ...
        writer.writerow([t, '{0:.4f}'.format(sensitivity), '{0:.4f}'.format(specificity), '{0:.4f}'.format(bal_acc),
                         '{0:.4f}'.format(acc), '{0:.4f}'.format(f1_score_1), '{0:.4f}'.format(mcc),
