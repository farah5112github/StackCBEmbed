...
def create_meta_layer(X, y):
    trainX, trainy = X, y
    model = SVC(kernel='rbf', random_state=1, probability=True, C=1, degree=3, gamma='scale')
    model.fit(trainX, trainy)
    if pssm and embedding:
        filename = f'./base_layer_pickle_files_with_PSSM/SVM_meta_layer.sav'
    elif embedding:
        filename = f'./base_layer_pickle_files_without_PSSM/SVM_meta_layer.sav'
    else:
        filename = f'./base_layer_pickle_files_without_embedding/SVM_meta_layer.sav'
    with open(filename, 'wb') as f:
        pickle.dump(model, f)
...
embedding = True
cmd = input("Enter 0 for without PSSM, 1 for with PSSM and 2 for without embedding: ")
if cmd == '0':
    pssm = False
elif cmd == '1':
    pssm = True
